...
import { createPIAims } from '../../../../api/PIApi';
import { getPIList, getPIAims } from '../../../../api/QueryProgramApi';
...
class CreatePIAims extends Component {
  ...
  handleOnOk = () => {
    const { form, piId, programId } = this.props;
    ...
    form.validateFields((err, values) => {
      if (!err) {
        ...
        createPIAims(piObjectiveDTO).then(() => {
          Promise.all([getPIList(programId), getPIAims(values.piId || piId, programId)]).then(([PIList, piAims]) => {
            PIStore.setPIList(PIList.map(item => (
              Object.assign(item, {
