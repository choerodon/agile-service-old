...
class SideBarContent extends Component {
    ...
    form.validateFields((err, values) => {
      if (!err) {
        ...
        const data = {
          ...
          sequence: KanbanStore.getBoardData.length - 1,
        };
        if (status) {
          confirm({
            title: '警告',
            ...
            onOk() {
              KanbanStore.axiosAddColumn(categoryCode, data).then((res2) => {
  ...
  handleAddStatus = (e) => {
    ...
    form.validateFields((err, values) => {
      if (!err) {
        ...
        KanbanStore.axiosAddStatus(params).then((data) => {
  ...
  renderOptions() {
    if (JSON.stringify(KanbanStore.getStatusCategory) !== '{}') {
      const status = KanbanStore.getStatusCategory.lookupValues;
  ...
  render() {
    ...
    const { name: kanbanName } = KanbanStore.getBoardList.get(KanbanStore.getSelectedBoard);
